...
void pixelsort_image(std::string& input, std::string& output, void (*mask_ptr)(char*, std::vector<pixel_stream>&, int&, int, int, int), void (*rotate_image)(char*, int&, int&, int&), float (*hbs)(char, char, char)) {
   ...
   if(format.empty()) {
     std::cout << "Image format is not supported! Supported format are png, pgm, ppm, jpg, jpeg, hdr and bmp" << std::endl;
     exit(1);
   }
   ...
   switch(const_hash(format.c_str())) {
     case const_hash("png"): std::cout << "png" << std::endl; break;
     case const_hash("pgm"): std::cout << "pgm" << std::endl; break;
     case const_hash("ppm"): std::cout << "ppm" << std::endl; break;
     case const_hash("jpg"): std::cout << "jpg" << std::endl; break;
     case const_hash("hdr"): std::cout << "hdr" << std::endl; break;
     case const_hash("bmp"): std::cout << "bmp" << std::endl; break;
   }

...
/*
  ...
  AVPixelFormat source_pix_fmt;

  int video_stream_index = -1;
  int stream_index = 0;
  int ret = 0;
  int frame_pts_counter = 0;
  ...
  int columns = dec_ctx->height;
  int rows = dec_ctx->width;
  int desired_comp = 3;
  ...
  while(av_read_frame(ifmt_ctx, dec_pkt) >= 0) {
	...
	//converting to rgb24
	sws_scale(sws_fmt_to_rgb, dec_frame->data, dec_frame->linesize, 0, dec_frame->height, dst_data, dst_linesize);
	...
	sws_scale(sws_rgb_to_fmt, dst_data, dst_linesize, 0, dec_frame->height, enc_frame->data, enc_frame->linesize);
...
//  	if(enc_frame && enc_frame->pts != AV_NOPTS_VALUE)
//  	  enc_frame->pts = av_rescale_q(enc_frame->pts, enc_frame->time_base, enc_ctx->time_base);
